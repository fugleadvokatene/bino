...
func AvailabilityDate(unavailablePeriods []Period) (enums.Availability, Date) {
	...
	for _, pv := range unavailablePeriods {
		if pv.From.Before(tomorrow) && tomorrow.Before(pv.To) {
			if pv.To.Year > tomorrow.Year+2 {
				return enums.AvailabilityUnavailableIndefinitely, pv.To
			}
			return enums.AvailabilityUnavailableUntil, pv.To
		}
		if tomorrow.Before(pv.From) {
			return enums.AvailabilityAvailableUntil, pv.From
		}
	}
	return enums.AvailabilityAvailableIndefinitely, tomorrow
}
