...
const getSortKey = (row: HTMLTableRowElement, col: number) => {
  ...
  return key !== undefined ? Number(key) : cell.textContent!.trim().toLowerCase()
}

// Sort the table
const sortTable = (col: number, th: HTMLTableCellElement) => {
  // Find tbody
  const tbody = QuerySelector<HTMLTableSectionElement>('.search-container tbody')!
...
const filterFormer = () => {
  ...
  let anyMatch = false
  boxes.forEach(box => {
    const match = QuerySelectorAll<HTMLElement>('.filter-content', box).some(el =>
